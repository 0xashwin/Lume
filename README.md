...
- Stremio addon support: add any addon by its `manifest.json` URL (or `stremio://` install link); its movie, series and TV catalogs browse like any other source, with streams resolved on demand at playback time
...
│    ├─ StremioClient       Stremio addon protocol          │
...
- **Xcode 26.4** or later
- An **Xtream Codes** account (server URL, username, password), an **M3U/M3U8 playlist URL**, a **Stalker portal** (portal URL + MAC address), or a **Stremio addon** (manifest URL)
